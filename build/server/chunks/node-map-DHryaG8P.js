...
const DB_URI = "mongodb://nnf:jakna@localhost:27017/?retryWrites=true&w=majority";
console.log("Connecting to database...");
console.log("DB_URI:", DB_URI);
const client = new MongoClient(DB_URI);
await client.connect();
const db = client.db("nnf-data");
const nodeMap = [
  {
...
async function getMappingToGameIds() {
  const nodeNaturalToGameIdMappingCollection = db.collection("nodeNaturalToGameId");
  for (const node of nodeMap) {
    ...
    if (!mapping) {
      ...
      await nodeNaturalToGameIdMappingCollection.insertOne({ natural_id, game_id: new_game_id });
    }
  }
  return await nodeNaturalToGameIdMappingCollection.find().toArray();
}
async function nodeByGameId(gameId) {
...
async function gameIdByNaturalId(naturalId) {
  const mapping = await getMappingToGameIds();
  return mapping.find((m) => m.natural_id === naturalId)?.game_id ?? null;
}

export { nodeMap as a, db as d, gameIdByNaturalId as g, nodeByGameId as n };
//# sourceMappingURL=node-map-DHryaG8P.js.map
